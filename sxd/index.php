...
class Sypex_Dumper
{
    function Sypex_Dumper()
    {
        ...
        $this->name = "Sypex Dumper Pro 2.0.11";
    ...
    function runServices($job)
    {
        ...
        if (isset($serv[$job["type"]])) {
            ...
            foreach ($tables AS $t) {
                    ...
                    case "correct":
                        ...
                        while ($col = mysql_fetch_assoc($c)) {
                            ...
                            if (mysql_query("ALTER TABLE {$t} CHANGE {$col['Field']} {$col['Field']} {$col['Type']} CHARACTER SET binary {$add}")) {
                                if (mysql_query("ALTER TABLE {$t} CHANGE `{$col['Field']}` `{$col['Field']}` {$col['Type']} CHARACTER SET {$job['charset']} COLLATE {$job['collate']} {$add}")) {
                                } else {
                                    $ok = false;
                                    break;
                                }
                ...
                if ($error)
                    echo "sxd.result.add(['{$t}', '{$type}', 'error', " . sxd_esc(mysql_error()) . "]);";
                else
    ...
    function addRestoreJob($job)
    {
        ...
        if (is_null($this->JOB["obj"])) {
            ...
            elseif (strpos($s, "\n"))
                ...
                $this->JOB["eol"] = "\r";
            $bom = strncmp($s, "пїњ", 3) == 0 ? 3 : ((strncmp($s, "ю€", 2) == 0 || strncmp($s, "€ю", 2) == 0) ? 2 : 0);
                ...
                switch ($q{0}) {
                    ...
                    case "I":
                        if (preg_match("/^(INSERT( INTO `(.+?)`).*?\\sVALUES)/s", $q, $m)) {
                            ...
                            $this->rtl[8] = 0;
                            if ($last_tab != $tab) {
                                $this->addLog(sprintf("ќтключаем индексы `%s`", $tab));
                    ...
                    case "D":
                        if (preg_match("/^DELIMITER (.+?)\\s/s", $q, $m)) {
                            ...
                            $delimiter = $m[1];
                            $this->addLog(sprintf("”становлен разделитель '%s'", $delimiter));
                        ...
                        if ($this->JOB["outfile"] == 1) {
                            ...
                            for ($i = 0; $i < $n[4]; $i += $limit) {
                                ...
                                if ($i)
                                    fwrite($this->fh_tmp, ",\n");
                                fwrite($this->fh_tmp, "(");
                                mysql_query("SELECT * INTO OUTFILE '{$this->JOB['file_buf']}' FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY \"'\" LINES TERMINATED BY '\\\0\\\0\\\0\\\0' FROM `{$n[1]}`" . ($n[5] < $buffer ? "" : " LIMIT {$i}, {$limit}")) or sxd_my_error();
                            ...
                            while ($row = mysql_fetch_row($r)) {
                                ...
                                for ($k = 0; $k < $fields; $k++) {
                                    if (!isset($row[$k])) {
                                        ...
                                        $row[$k] = "'" . mysql_real_escape_string($row[$k]) . "'";
                                    }
                                }
                                $fcache .= "(" . implode(",", $row) . "),\n";
                                $this->rtl[7]++;
    ...
    function setNames($collation)
    {
        ...
        if ($this->rtl[6] != $collation) {
            $query = $this->query;
            $query("SET NAMES '" . preg_replace("/^(\\w+?)_/", "\\1' COLLATE '\\1_", $collation) . "'");
            if (!$this->rtl[7])
