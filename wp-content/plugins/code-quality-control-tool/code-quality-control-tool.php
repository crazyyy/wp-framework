  ...
  */
  ...
  if (is_admin()) {
    ...
    function cqctphp_admin_init() {
      ...
      wp_enqueue_style('cqctphp_Load_CSS', plugins_url('css/style.css'));
    }
    ...
    function cqctphp_frontend_shortcut() {
      ...
      if ($errors_count > 0) {
        $alert_html = ' <span class="numcirc">'.$errors_count.'</span>';
      } else {
        $alert_html = '';
      }
      ...
      $wp_admin_bar->add_menu(array(
        ...
        'href'  => get_admin_url(
          null,
          'options-general.php?page=php-code-control-settings'
        ),
        'meta'  => array(
          'tabindex' => 0,
          'class'    => 'code-control-top-toolbar'
        ),
      ));
    }
    ...
    function cqctphp_download_file() {
      if (isset($_POST['action']) && $_POST['action'] == 'download_log' && check_admin_referer(
          'cqctphp_save_settings_BF944B'
        )) {
    ...
    function cqctphp_register_page_settings() {
      add_options_page(
        ...
        'PHP Code Control',
        'manage_options',
        'php-code-control-settings',
        'cqctphp_page_settings'
      );
    }
    ...
    function cqctphp_page_settings() {
        ...
        switch ($action) {
          ...
          case 'save_settings':
            ...
            if (in_array("ALL", $settings['object_check'])) {
              $settings['object_check'] = array("ALL");
            }
            $settings['object_check'] = array_values(
              array_filter($settings['object_check'])
            );
      ...
      if ($settings['is_active'] == 0) {
        $html_logger_status = '<span class="numcirc">Logger is disabled</span>';
      } else {
        $html_logger_status = '<span class="numcirc greennumcirc">Logger is active</span>';
      }
      ?>
      <h1>PHP Code Control <?php
          echo $html_logger_status; ?></h1>
      ...
      <p>Checks the PHP code quality of installed plugins and themes on your
         server PHP version.</p>

      <p>Your PHP version: <b><?php
            echo phpversion(); ?></b></p>
      <?php
      $errors_count = PHPCodeControl_general::GetErrorCount();
      if ($errors_count > 0) {
        $html_counter = '<span class="numcirc">'.$errors_count.'</span>';
      } else {
        $html_counter = $errors_count;
      }
      ?>
      <p>Total detected issues: <b><?php
            echo $html_counter; ?></b></p>
      <?php
      if ($errors_count > 0) {
        ?>
        <p><b>Please note:</b> Errors/Notices/Warnings are there for a reason to
                               inform you that something is not right.
                               Treat them as such and don't just act like they
                               don't exist. At a moment you'll rewrite some
                               code, just a little
                               bit, a weird bug will appear in a specific case.
                               You'll find it later and it could do bad stuff to
                               your
                               application.</p>
        <p>We recomend to contact your developers. If you don't any advanced
           developers contact <a
            href="<?php
              echo $support_link; ?>" target="_blank">SafetyBis.com</a></p>
        <p>
          <a href="<?php
            echo $support_link; ?>" target="_blank">
            <img src="<?php
              echo plugins_url('images/livechat.png', __FILE__); ?>"/>
          </a>
        </p>
        <?php
      }
      ...
      <form method="post" action="options-general.php?page=php-code-control-settings">
        ...
        <table class="form-table" role="presentation">
            ...
            <td>
              <?php
                ...
                foreach ($list as $v) {
                  ...
                  <label for="type_<?php
                    echo $v; ?>">
                    <input class="errortypes <?php
                      echo $v; ?>" name="errortypes[]" type="checkbox"
                           id="type_<?php
                             echo $v; ?>"
                           value="<?php
                             echo $v; ?>" <?php
                      if (in_array($v, $selected_list)) {
              ...
              ?>
              <p>
                <a href="javascript:" onclick="ManageErrorTypes('uncheck')">Uncheck
                                                                            All</a>&nbsp;&nbsp;|&nbsp;&nbsp;
                <a href="javascript:" onclick="ManageErrorTypes('all')">Select
                                                                        All</a>&nbsp;&nbsp;|&nbsp;&nbsp;
                <a href="javascript:" onclick="ManageErrorTypes('error')">Select
                                                                          ERROR
                                                                          only</a>&nbsp;&nbsp;|&nbsp;&nbsp;
                <a href="javascript:" onclick="ManageErrorTypes('warning')">Select
                                                                            WARNING
                                                                            only</a>&nbsp;&nbsp;|&nbsp;&nbsp;
                <a href="javascript:" onclick="ManageErrorTypes('notice')">Select
                                                                           NOTICE
                                                                           only</a>
              </p>
              <p>Error handling is the process of catching errors raised by your
                 program and then taking appropriate
                 action. If you would handle errors properly then it may lead to
                 many unforeseen consequences.</p>
              <p>For more information please read
                <a href="https://www.php.net/manual/en/errorfunc.constants.php"
                   target="_blank">https://www.php.net/manual/en/errorfunc.constants.php</a>
              </p>
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>
              <input name="logsize" type="number" step="1" min="0" value="<?php
                echo $settings['logsize']; ?>"
                     class="small-text"> Mb (0 for unlimited)
              <br>
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>
              <select name="skip_dups">
                <option <?php
                  if ($settings['skip_dups'] == 0) {
                    echo 'selected="selected"';
                  } ?> value="0">Log all errors
                </option>
                <option <?php
                  if ($settings['skip_dups'] == 1) {
                    echo 'selected="selected"';
                  } ?> value="1">Log uniq errors
                                 only (skip dups)
                </option>
              </select>
              <br>
              <p>Skip dups - will skip logging if error is already logged
                 before</p>
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>
              <p>
              <textarea name="filer_by_ip" id="filer_by_ip" rows="5" cols="50"
                        class="large-text code"><?php
                  if (isset($settings['filer_by_ip']) && is_array(
                      $settings['filer_by_ip']
                    )) {
                    echo implode("\n", $settings['filer_by_ip']);
                  } ?></textarea>
              </p>
              <p>It will save logs for specific IP addresses only (one IP per
                 row)</p>
              <p>Your current IP is <b><?php
                    echo $_SERVER['REMOTE_ADDR']; ?></b> <a href="javascript:"
                                                            onclick="AddMyIP()">[Add
                                                                                to
                                                                                List]</a>
              </p>
            ...
            <td>
              <select name="object_check[]" id="object_check" onchange="ManageThemesPlugins()">
                <option <?php
                  ...
                  Trace everything (plugins, themes and WordPress core files)
                </option>
                <option <?php
                  if ( ! in_array("ALL", $settings['object_check'])) {
                    echo 'selected="selected"';
                  } ?> value="">
                  Trace selected objects only
                </option>
              </select>
              <br>
            </td>
            ...
            <td>
              <?php
                ...
                foreach ($list as $v) {
                  ...
                  <label for="type_<?php
                    echo $v['theme_slug']; ?>">
                    <input class="obj_themes" name="object_check[]" type="checkbox"
                           id="type_<?php
                             echo $v['theme_slug']; ?>"
                           value="<?php
                             echo $v['theme_path']; ?>" <?php
                      if (in_array(
                        $v['theme_path'],
                    ...
                    <?php
                      echo $v['theme_name'].' ('.$v['theme_slug'].')'; ?>
                  </label>
                  <br>
                  <?php
                }
              ?>
              <p>
                <a href="javascript:" onclick="ManageThemes('uncheck')">Uncheck
                                                                        All</a>&nbsp;&nbsp;|&nbsp;&nbsp;
                <a href="javascript:" onclick="ManageThemes('all')">Select
                                                                    All</a>
              </p>
            ...
            <td>
              <?php
                ...
                foreach ($list as $v) {
                  ...
                  <label for="type_<?php
                    echo $v['plugin_slug']; ?>">
                    <input class="obj_plugins" name="object_check[]" type="checkbox"
                           id="type_<?php
                             echo $v['plugin_slug']; ?>"
                           value="<?php
                             echo $v['plugin_path']; ?>" <?php
                      if (in_array(
                        $v['plugin_path'],
                        $settings['object_check']
                      )) {
                    ...
                    <?php
                      echo $v['plugin_name'].' ('.$v['plugin_slug'].')'; ?>
                  </label>
                  <br>
                  <?php
                }
              ?>
              <p>
                <a href="javascript:" onclick="ManagePlugins('uncheck')">Uncheck
                                                                         All</a>&nbsp;&nbsp;|&nbsp;&nbsp;
                <a href="javascript:" onclick="ManagePlugins('all')">Select
                                                                     All</a>
              </p>
        ...
        <script>
          ...
          function ManageErrorTypes (t) {
            ...
            if (t == 'all') {
              jQuery('.errortypes').prop('checked', true)
            }
            if (t == 'error') {
              jQuery('.errortypes').prop('checked', false)
          ...
          function ManageThemes (t) {
            if (t == 'uncheck') {
              jQuery('.obj_themes').prop('checked', false)
            }
            if (t == 'all') {
              jQuery('.obj_themes').prop('checked', true)
            }
          }

          function ManagePlugins (t) {
            if (t == 'uncheck') {
              jQuery('.obj_plugins').prop('checked', false)
            }
            if (t == 'all') {
              jQuery('.obj_plugins').prop('checked', true)
            }
          }
        </script>
      ...
      <a href="javascript:" <?php
        if ( ! $is_disabled) {
          echo 'onclick="FormActions(\'download_log\');"';
        } ?>
         class="button action" <?php
        if ($is_disabled) {
      ...
      <a href="javascript:" <?php
        if ( ! $is_disabled) {
          echo 'onclick="FormActions(\'clear_log\');"';
        } ?>
         class="button action" <?php
        if ($is_disabled) {
          echo 'disabled="disabled"';
      ...
      if ($total_lines > 0) {
        if ($total_lines > 100) {
          ?>
          <p>If you need to see all <?php
              echo $total_lines; ?> lines of log, please download the log
             file.</p>
          <?php
        }
        ?>
          ...
          <?php
            ...
            if (count($lines)) {
              foreach ($lines as $line) {
                ...
                <tr>
                  <td><?php
                      echo $line[0]."<br>".$line[1]; ?></td>
                  <td><?php
                      echo $line[2]."<br>".$line[5]; ?></td>
                  <td><?php
                      echo $line[3]."<br>".$line[4]."<br>".$line[6]; ?></td>
                </tr>
                <?php
                $i--;
                if ($i == 0) {
  ...
  class PHPCodeControl_general {
    ...
    public static function Clear_Log_File() {
      ...
      if (file_exists($log_counter_file)) {
        unlink($log_counter_file);
      }
    }
    ...
    public static function LoadSettings() {
      ...
      if ( ! isset($settings['object_check']) || ! is_array(
          $settings['object_check']
        )) {
        $settings['object_check'] = array("ALL");
      }
    ...
    public static function build_ini_string(array $a) {
      ...
      foreach ($a as $rootkey => $rootvalue) {
        if (is_array($rootvalue)) {
          ...
          $indexed_root = array_keys($rootvalue) == range(
              0,
              count($rootvalue) - 1
            );
    ...
    public static function Get_List_WP_Themes() {
      ...
      foreach ($themes as $theme_slug => $theme_block) {
        ...
        $result[] = array(
          ...
          'theme_path' => str_replace(
            ABSPATH,
            "",
            $theme_info->theme_root.'/'.$theme_slug
          ),
      ...
      foreach ($plugins as $plugin_file => $plugin_block) {
        $result[] = array(
          ...
          'plugin_path' => str_replace(
            ABSPATH,
            "",
            WP_CONTENT_DIR.'/plugins/'.dirname($plugin_file)
          ),
