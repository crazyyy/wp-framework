...
class ACFCG_Group {
	...
	public $clone = false;
	public $clone_parent_ACFCG_group = null;
	...
	function __construct( $args = array() ) {
		...
		$this->clone_parent_ACFCG_group = $args['clone_parent_ACFCG_group'];
	...
	*/
	private function get_fields() {
		...
		if ( 'postmeta' == ACFCG_Core::$db_table ) { // ACF
			return $this->get_fields_from_postmeta_table();
		 } elseif ( 'posts' == ACFCG_Core::$db_table ) { // ACF PRO
	...
	public function get_field_group_html() {
		...
		if ( 'postmeta' == ACFCG_Core::$db_table ) {
			...
			foreach ( $this->fields as $field ) {
				...
				$field_class_name = ACFCG_Core::$class_prefix . 'Field';
				$ACFCG_field = new $field_class_name( $args );

				array_push( $ACFCG_fields, $ACFCG_field );

			}

			// sort fields
			usort( $ACFCG_fields, array( $this, "compare_field_order") );

			// render fields
			foreach ( $ACFCG_fields as $ACFCG_field ) {
				$field_group_html .= $ACFCG_field->get_field_html();
			}
		...
		elseif ( 'posts' == ACFCG_Core::$db_table ) {
			...
			foreach ( $this->fields as $field_post_obj ) {
				...
				$args = array(
					...
					'clone_parent_ACFCG_field' => $this->clone_parent_ACFCG_group, // TODO: Add this clone bit to the postmeta table func above?
					'exclude_html_wrappers' => $this->exclude_html_wrappers
				);
