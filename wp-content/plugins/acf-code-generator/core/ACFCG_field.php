...
class ACFCG_Field {
	...
	function __construct( $args = array() ) {
		...
		if ( "postmeta" == ACFCG_Core::$db_table ) {
			$this->construct_from_postmeta_table( $args['field_data_obj'] );
		} elseif ( "posts" == ACFCG_Core::$db_table ) {
		...
		// cloned fields
		if ( $args['clone_parent_ACFCG_field'] ) {

			$this->clone = true;
			$clone_settings = $args['clone_parent_ACFCG_field']->settings;

			// Reset location rule paramater
			$this->location_rule_param = $args['location_rule_param'];

			if ( 1 === $clone_settings['prefix_name'] ) {
				$this->name = $args['clone_parent_ACFCG_field']->name . '_' . $this->name;
			}
	...
	protected function get_render_partial() {
		...
		if ( $this->type ) {
            ...
            if ( in_array( $this->type, ACFCG_Core::$field_types_basic ) ) {

                $render_partial = ACFCG_PLUGIN_DIR_PATH . 'render/basic.php';
			...
			// Field types with their own partial
                ...
                $render_partial = ACFCG_PLUGIN_DIR_PATH . 'render/' . $this->type . '.php';
	...
	protected function is_ignored_field_type( $field_type = '' ) {
		...
		return in_array( $field_type, ACFCG_Core::$ignored_field_types ); 
...
<?php
	...
	protected function get_field_html_body() {
		...
		} elseif ( in_array( $this->type, ACFCG_Core::$field_types_all_tc_pro ) ) {

			echo $this->indent . htmlspecialchars( "<?php // Upgrade to ACF Code Generator Pro for " . $this->type . " field support. ?>" ) . "\n";
			echo $this->indent . htmlspecialchars( "<?php // Visit https://tutsocean.com for more information. ?>" ) . "\n";
			
		} else {
			
			echo $this->indent . htmlspecialchars( "<?php // The " . $this->type  . " field type is not supported in this version of the plugin. ?>" ) . "\n";
			echo $this->indent . htmlspecialchars( "<?php // Contact https://tutsocean.com to request support for this field type. ?>" ) . "\n";
