...
$cssUrl = 'css/pQp.css'; //$config.'css/pQp.css';
...
<script type="text/javascript">
	...
	function loadCSS() {
		...
		sheet.setAttribute("href", "<?php echo WP_PLUGIN_URL.'/wp-developer-tools/'.$cssUrl; ?>");
	...
	echo '<table class="side" cellspacing="0">
		...
		foreach($output['logs']['console'] as $log) {
			echo '<tr class="log-'.$log['type'].'">
				...
				<td class="'.$class.'">';
			if($log['type'] == 'log') {
				echo '<div><em>'.$log['name'].'<em><pre>'.$log['data'].'</pre></div>';
			}
...
if($output['queryTotals']['count'] ==  0) {
	...
	echo '<table class="side" cellspacing="0">
		  ...
		  <tr><td><var>'.$output['queryTotals']['dupes'].'</var> <h4>Duplicates</h4></td></tr>
		  <tr><td class="alt"><var>'.$output['queryTotals']['size'].' bytes</var> <h4>Total Size</h4></td></tr>
		 </table>
		<table class="main" cellspacing="0">';
		...
		foreach($output['queries'] as $query) {
			...
			echo '<em>Size: '.$query['size'].' bytes</em>';
			echo '</td></tr>';
			if($class == '') $class = 'alt';
			else $class = '';
		}
