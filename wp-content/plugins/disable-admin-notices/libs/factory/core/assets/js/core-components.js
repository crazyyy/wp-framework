...
(function($) {
	...
	var externalAddon = {
		...
		events: function() {
			...
			$(document).on('click', '.wfactory-453-process-button', function() {
				...
				self.sendRequest(data, function(response) {
					...
					$.wfactory_453.hooks.run('core/components/updated', [$this, data, response]);
				});
		...
		sendRequestToComponentActivationPrepare: function(componentButton, sendData, button_i18n) {
			...
			sendData.action = 'wfactory-453-prepare-component';
			...
			this.sendRequest(sendData, function(response) {
				...
				if( !response || !response.success ) {
					...
					$.wfactory_453.hooks.run('core/components/activation_error', [
						componentButton,
						sendData,
						response
					]);
				...
				$.wfactory_453.hooks.run('core/components/activated', [
					componentButton,
					sendData,
					response
				]);
		...
		sendRequest: function(data, callback) {
			...
			$.ajax(ajaxurl, {
				...
				error: function(xhr, ajaxOptions, thrownError) {
					...
					$.wfactory_453.hooks.run('core/components/ajax_error', [
						xhr,
						ajaxOptions,
						thrownError
					]);
