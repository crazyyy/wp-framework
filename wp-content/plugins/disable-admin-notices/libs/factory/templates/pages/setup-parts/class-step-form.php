...
class Step_Form extends Step {
	...
	public function __construct(\WBCR\Factory_Templates_127\Pages\Setup $page)
	{
	...
	protected function instance_form($options)
	{
		...
		$form = new \Wbcr_FactoryForms474_Form([
			'scope' => rtrim($this->plugin->getPrefix(), '_'),
			'name' => $this->page->getResultId() . "-options-" . $this->get_id()
		], $this->plugin);

		$form->setProvider(new \Wbcr_FactoryForms474_OptionsValueProvider($this->plugin));
	...
	protected function render_form(\Wbcr_FactoryForms474_Form $form)
	{
		?>
		<form method="post" id="w-factory-templates-127__setup-form-<?php echo $this->get_id() ?>" class="w-factory-templates-127__setup-form form-horizontal">
			<?php $form->html(); ?>
			<div class="w-factory-templates-127__form-buttons">
				<!--<input type="submit" name="skip_button_<?php /*echo $this->get_id() */ ?>" class="button-primary button button-large w-factory-templates-127__skip-button" value="<?php /*_e('Skip', 'wbcr_factory_templates_127') */ ?>">-->
				<input type="submit" name="continue_button_<?php echo $this->get_id() ?>" class="button-primary button button-large w-factory-templates-127__continue-button" value="<?php _e('Continue', 'wbcr_factory_templates_127') ?>">
			</div>
		</form>
		<?php
	}

	protected function set_form_handler(\Wbcr_FactoryForms474_Form $form)
	{
	...
	public function html()
	{
		...
		<div id="WBCR" class="wrap">
			<div class="wbcr-factory-templates-127-impressive-page-template factory-bootstrap-476 factory-fontawesome-000">
