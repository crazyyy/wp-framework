...
<div id="login">
	...
	<p id="backtoblog"><a href="<?php echo esc_url(home_url('/')); ?>"><?php
		...
		printf(esc_html_x('&larr; Back to %s', 'site', 'all-in-one-wp-security-and-firewall'), esc_html(get_bloginfo('title', 'display')));
	?></a></p>
	<?php
...
switch ($action) {
	...
	if (isset($_GET['error'])) {
			...
			$errors->add('invalidkey', __('Your password reset link appears to be invalid.', 'all-in-one-wp-security-and-firewall') . ' ' . __('Please request a new link below.', 'all-in-one-wp-security-and-firewall'));
				} elseif ('expiredkey' == $_GET['error']) {
			$errors->add('expiredkey', __('Your password reset link has expired.', 'all-in-one-wp-security-and-firewall') . ' ' . __('Please request a new link below.', 'all-in-one-wp-security-and-firewall'));
				}
		}
	...
	login_header(__('Lost Password', 'all-in-one-wp-security-and-firewall'), '<p class="message">' . __('Please enter your username or email address.', 'all-in-one-wp-security-and-firewall') . ' ' . __('You will receive a link to create a new password via email.', 'all-in-one-wp-security-and-firewall') . '</p>', $errors);
	...
	?>
		...
		<form name="lostpasswordform" id="lostpasswordform" action="<?php echo esc_url(network_site_url('wp-login.php?action=lostpassword', 'login_post')); ?>" method="post">
			<p>
				<label for="user_login" ><?php esc_html_e('Username or Email Address', 'all-in-one-wp-security-and-firewall'); ?><br />
	...
	do_action('lostpassword_form'); ?>
		...
		<p id="nav">
			<a href="<?php echo esc_url(wp_login_url()); ?>"><?php esc_html_e('Log in', 'all-in-one-wp-security-and-firewall'); ?></a>
			<?php
	...
	?>
		<form name="resetpassform" id="resetpassform" action="<?php echo esc_url(network_site_url('wp-login.php?action=resetpass', 'login_post')); ?>" method="post" autocomplete="off">
			...
			<div class="user-pass1-wrap">
					<p>
							<label for="pass1"><?php esc_html_e('New password', 'all-in-one-wp-security-and-firewall'); ?></label>
					</p>
					...
					<div class="wp-pwd">
							...
							<div id="pass-strength-result" class="hide-if-no-js" aria-live="polite"><?php esc_html_e('Strength indicator', 'all-in-one-wp-security-and-firewall'); ?></div>
					</div>
					<div class="pw-weak">
							<label>
									<input type="checkbox" name="pw_weak" class="pw-checkbox" />
	<?php esc_html_e('Confirm use of weak password', 'all-in-one-wp-security-and-firewall'); ?>
							</label>
					</div>
			</div>
			<p class="user-pass2-wrap">
				<label for="pass2"><?php esc_html_e('Confirm new password', 'all-in-one-wp-security-and-firewall'); ?></label><br />
	...
	?>
		...
		<form name="registerform" id="registerform" action="<?php echo esc_url(site_url('wp-login.php?action=register', 'login_post')); ?>" method="post" novalidate="novalidate">
			<p>
				<label for="user_login"><?php esc_html_e('Username', 'all-in-one-wp-security-and-firewall'); ?><br />
					<input type="text" name="user_login" id="user_login" class="input" value="<?php echo esc_attr(wp_unslash($user_login)); ?>" size="20" /></label>
			</p>
			<p>
				<label for="user_email"><?php esc_html_e('Email', 'all-in-one-wp-security-and-firewall'); ?><br />
	...
	?>
			<p id="reg_passmail"><?php esc_html_e('Registration confirmation will be emailed to you.', 'all-in-one-wp-security-and-firewall'); ?></p>
			<br class="clear" />
			<input type="hidden" name="redirect_to" value="<?php echo esc_attr($redirect_to); ?>" />
	...
	<?php echo esc_html($login_link_separator); ?>
			<a href="<?php echo esc_url(wp_lostpassword_url()); ?>"><?php esc_html_e('Lost your password?', 'all-in-one-wp-security-and-firewall'); ?></a>
		</p>
	...
	if (! isset($_GET['request_id'])) {
			wp_die(esc_html__('Invalid request.', 'all-in-one-wp-security-and-firewall'));
		}
	...
	if (empty($_COOKIE[LOGGED_IN_COOKIE])) {
			...
			$user = new WP_Error('test_cookie', sprintf(__('%1$s: Cookies are blocked due to unexpected output.', 'all-in-one-wp-security-and-firewall') . ' ' . __('For help, please see %2$s or try the %3$s.', 'all-in-one-wp-security-and-firewall'), '<strong>' . __('ERROR', 'all-in-one-wp-security-and-firewall') . '</strong>', '<a href="https://codex.wordpress.org/Cookies">' . __('this documentation', 'all-in-one-wp-security-and-firewall') . '</a>', '<a href="https://wordpress.org/support/">' . __('support forums', 'all-in-one-wp-security-and-firewall') . '</a>'));
				} elseif (isset($_POST['testcookie']) && empty($_COOKIE[TEST_COOKIE])) {
			// If cookies are disabled we can't log in even with a valid user+pass
			// translators: 1: 'ERROR', 2: 'enable cookies'(Browser cookie documentation link)
			$user = new WP_Error('test_cookie', sprintf(__('%1$s: Cookies are blocked or not supported by your browser.', 'all-in-one-wp-security-and-firewall') . ' ' . __('You must %2$s to use WordPress.', 'all-in-one-wp-security-and-firewall'), '<strong>' . __('ERROR', 'all-in-one-wp-security-and-firewall') . '</strong>', '<a href="https://codex.wordpress.org/Cookies">' . __('enable cookies', 'all-in-one-wp-security-and-firewall') . '</a>'));
				}
		}
	...
	if ($interim_login) {
		...
		$errors->add('registered', __('Registration complete.', 'all-in-one-wp-security-and-firewall') . ' ' . __('Please check your email.', 'all-in-one-wp-security-and-firewall'), 'message');
...
<form name="loginform" id="loginform" action="<?php echo esc_url(site_url('wp-login.php', 'login_post')); ?>" method="post">
	...
	<p>
		<label for="user_pass"><?php esc_html_e('Password', 'all-in-one-wp-security-and-firewall'); ?><br />
